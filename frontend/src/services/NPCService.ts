...
export class NPCService {
  ...
  private behaviors: Map<string, NPCBehavior[]> = new Map(); // npcId -> behaviors
  private aiStates: Map<string, NPCAIState> = new Map();
  private eventListeners: Map<NPCEventType, Function[]> = new Map();
  ...
  private initializeEventListeners(): void {
    Object.values(NPC_EVENTS).forEach(event => {
  ...
  private async loadNPCs(): Promise<void> {
    ...
    // For now, we'll create some sample NPCs
  ...
  private async loadBehaviors(): Promise<void> {
    // Load NPC behaviors from configuration
  ...
  private async loadAIStates(): Promise<void> {
    // Initialize AI states for each NPC
  ...
  private executeBehavior(behavior: NPCBehavior): void {
    ...
    this.emit(NPC_EVENTS.NPC_BEHAVIOR_TRIGGERED, { behavior, npc });
  }
  ...
  public startConversation(npcId: string, playerId: string): NPCConversation {
    ...
    this.emit(NPC_EVENTS.NPC_INTERACTION_START, { conversation, npc });
    return conversation;
  }
  ...
  public endConversation(conversationId: string): void {
    ...
    if (npc) {
      const farewell = npc.dialogue.farewells[Math.floor(Math.random() * npc.dialogue.farewells.length)];
      this.emit(NPC_EVENTS.NPC_DIALOGUE, { npc, message: farewell });
    }

    this.emit(NPC_EVENTS.NPC_INTERACTION_END, { conversation, npc });
  }
  ...
  public sendMessage(conversationId: string, message: string, speaker: 'npc' | 'player'): void {
    ...
    this.emit(NPC_EVENTS.NPC_DIALOGUE, { conversation, message: conversationMessage });
  }
  ...
  // Event system
  public on(event: NPCEventType, callback: Function): void {
    const listeners = this.eventListeners.get(event) || [];
    listeners.push(callback);
    this.eventListeners.set(event, listeners);
  }

  public off(event: NPCEventType, callback: Function): void {
    const listeners = this.eventListeners.get(event) || [];
  ...
  private emit(event: NPCEventType, data?: any): void {
    const listeners = this.eventListeners.get(event) || [];
